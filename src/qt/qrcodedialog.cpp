...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non FOX unit input in lnReqAmount, we generate the URI with FOX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(FoxcoinUnits::format(FoxcoinUnits::FOX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
